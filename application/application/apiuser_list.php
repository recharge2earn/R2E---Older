...
class Apiuser_list extends CI_Controller {
	...
	{ 
		...
		$result = $this->db->query("select user_id from tblusers where usertype_name = 'APIUSER'");
		...
		$config['base_url'] = base_url()."apiuser_list/pageview";
		...
		$this->view_data['result_dealer'] = $this->db->query("select tblusers.*,(select state_name from tblstate where tblstate.state_id = tblusers.state_id) as state_name,(select state_name from tblcity where tblcity.city_id = tblusers.city_id) as city_name from tblusers where usertype_name = 'APIUSER' order by business_name limit ?,?",array($start_row,$per_page));

		$this->view_data['message'] =$this->msg;

		$this->load->view('apiuser_list_view',$this->view_data);		
...
$this->output->set_header("Pragma: no-cache"); 
		...
		if($this->input->post("hidSearchFlag") && $this->input->post("hidSearchValue"))

			{
				...
				$result = $this->Md_dealer_list_model->getMasterdealerFiltered("APIUSER",$SearchBy,$SearchWord);		
			...
			{								
...

				{
					...
					redirect(base_url()."apiuser_list/pageview/".$start_page);
