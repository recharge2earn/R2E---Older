...
class D_registration extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache");
		...
		if($this->input->post("btnSubmit")){
				...
				{
				$this->load->model('d_registration_model');
				if($this->d_registration_model->find_mobile_exist($MobNo))
				{
					$user_id=$this->d_registration_model->add($Distname,$Parent_id,$PostalAddr,$Landmark,$Pin,$State,$City,$Area,$ConPer,$MobNo,$LandNo,$RetType,$Email,$Other_Area,$Bank1,$AcNo1,$AcType1,$Org,$PreLang,$Bank2,$AcNo2,$AccType2,$Scheme_id,$PayMode,$ChqDDNo,$ChqDDDate,$DepBank,$WorLimit,$Scheme_amt);
					$UserName = "A2Z".$user_id;
					$Password = $this->common->GetPassword();
					$this->d_registration_model->update($UserName,$Password,$user_id);
					//$to = $Email;
...
'Your account has been successfully created.User Name : '.$UserName.'Password : '.$Password.'www.a2zpay.biz';
		...
		{$data['message']='';$this->load->view('d_registration_view',$data);}
	}
	}
}
?>