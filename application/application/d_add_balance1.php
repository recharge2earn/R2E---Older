...
class D_add_balance1 extends CI_Controller {
	private $msg='';
	public function process()
	{
		if($this->input->post('btnSubmit'))
		{
			...
			{
				$this->load->model('D_add_balance1_model');
				if($this->D_add_balance1_model->add_newbalance($cr_user_id,$amount) == true)
				{
				$user_info = $this->D_add_balance1_model->GetUserInfo($cr_user_id);
				$dist_user_info = $this->D_add_balance1_model->GetUserInfo($this->session->userdata('id'));
				$this->load->model('Recharge_home_model');
...
'Dear Business Partner,Your Received Amount is '.$amount.',Your Updated Balance is '.($retailer_balance).',www.akashmobileworld.com';
		...
		$this->load->model('D_add_balance1_model');
		$this->view_data['result_retailer'] =$this->D_add_balance1_model->get_retailer($user_id);
		$this->load->model('Recharge_home_model');
	...
	public function back()
	{
		if($this->input->post('btnSubmit'))
		{
			...
			{
				$this->load->model('D_add_balance1_model');
				if($this->D_add_balance1_model->add_revert_balance($cr_user_id,$dr_user_id,$amount) == true)
				{
				$user_info = $this->D_add_balance1_model->GetUserInfo($dr_user_id);
				$dist_user_info = $this->D_add_balance1_model->GetUserInfo($cr_user_id);
				$this->load->model('Recharge_home_model');
				$retailer_balance = $this->Recharge_home_model->GetBalanceByUser($dr_user_id);
