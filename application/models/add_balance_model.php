<?php
class Add_balance_model extends CI_Model 
{	
	function _construct()
	{
		  // Call the Model constructor
		  parent::_construct();
	}
	public function GetUserInfo($user_id)
	{
		$str_query = "select * from tblusers where `tblusers`.user_id=?";
		$result = $this->db->query($str_query,array($user_id));		
		return $result;
	}
	
	public function add_ForexIncome($user_id,$amount,$remark)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array('1',$date,$ip));
		$master_id = $this->db->insert_id();		
		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress) values(?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($user_id,'1',$master_id,$amount,'cash',$remark,'ForexIncome',$payment_date,$payment_time,$date,$ip));
					
	}

	public function add_newbalance($cr_user_id,$amount)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array($this->session->userdata('id'),$date,$ip));
		$master_id = $this->db->insert_id();
		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress) values(?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,$this->session->userdata('id'),$master_id,$amount,'cash','Direct Payment','Recharge',$payment_date,$payment_time,$date,$ip));
		
		if($result_cr_dr > 0)
		{
			return true;
		}
		else
		{
			return false;
		}		
	}	
	public function add_MLMIncome($cr_user_id,$amount,$refer_user_id)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array($this->session->userdata('id'),$date,$ip));
		$master_id = $this->db->insert_id();
		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress,ref_id) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,$this->session->userdata('id'),$master_id,$amount,'cash','To Join New MLM User Income','ReferUser',$payment_date,$payment_time,$date,$ip,$refer_user_id));
		
		if($result_cr_dr > 0)
		{
			return true;
		}
		else
		{
			return false;
		}		
	}		
	
	public function add_MLMIncome3($cr_user_id,$dr_user_id,$amount)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array($dr_user_id,$date,$ip));
		$master_id = $this->db->insert_id();		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress) values(?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,$dr_user_id,$master_id,$amount,'cash','Load Income','TrensferComm',$payment_date,$payment_time,$date,$ip));
		
		if($result_cr_dr > 0)
		{
			return true;
		}
		else
		{
			return false;
		}		
	}	
	
	
	public function add_MLMIncome2($cr_user_id,$amount,$refer_user_id1)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array($this->session->userdata('id'),$date,$ip));
		$master_id = $this->db->insert_id();
		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress,ref_id) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,'1',$master_id,$amount,'cash','Joining Income','ReferUser',$payment_date,$payment_time,$date,$ip,$refer_user_id1));
		
		if($result_cr_dr > 0)
		{
			return true;
		}
		else
		{
			return false;
		}		
	}	
	
	public function add_revert_balance($cr_user_id,$dr_user_id,$amount)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		$payment_date = $this->common->getMySqlDate();
		$payment_time = $this->common->getMySqlTime();						
		
		$str_master_query = "insert into tblpayment_master(user_id,add_date,ip_address) values(?,?,?)";
		$result_master = $this->db->query($str_master_query,array($cr_user_id,$date,$ip));
		$master_id = $this->db->insert_id();
		
		
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress) values(?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,$dr_user_id,$master_id,$amount,'cash','Revert Back Direct Payment','Recharge',$payment_date,$payment_time,$date,$ip));
		if($result_cr_dr > 0)
		{
			return true;
		}
		else
		{
			return false;
		}		
	}
	
}

?>