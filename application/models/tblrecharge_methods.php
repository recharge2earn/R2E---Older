...
{	
	...
	{
		$rlst = $this->db->query("select tblrecharge.*,(select company_name from tblcompany where tblcompany.company_id = tblrecharge.company_id) as company_name,(select business_name from tblusers where tblusers.user_id = tblrecharge.user_id ) as business_name,(select business_name from tblusers where tblusers.user_id = (select parent_id from tblusers where tblusers.user_id = tblrecharge.user_id ) ) as parent_name from tblrecharge where recharge_id = '".$rechargeId."'");
		return $rlst;
	}
	...
	{
		$rlst = $this->db->query("select business_name from tblusers where user_id = (select user_id from tblrecharge where tblrecharge.recharge_id = '".$recharge_Id."')");
		return $rlst->row(0)->business_name;
	}
}
