<?php
class Franchise_registration_model extends CI_Model 
{	
	function _construct()
	{
	...
	public	function add($Franchisename,$Address,$Landmark,$Zipcode,$State,$City,$Area,$ResPer,$MobNo,$LandNo,$RetType,$Email,$Other_Area,$Bank1,$AcNo1,$AcType1,$Org,$PreLang,$Bank2,$AcNo2,$AccType2,$Scheme_id,$PayMode,$ChqDDNo,$ChqDDDate,$DepBank,$WorLimit,$Scheme_amt)
	{
		...
		$str_query = "insert into tblusers(business_name,postal_address,landmark,pincode,state_id,city_id,subarea_id,contact_person,mobile_no,landline,retailer_type_id,emailid,other_area,usertype_name,add_date,ipaddress,bank_id,account_no,account_type,ordganisation,prefered_language,bank_id_2,account_no_2,account_type_2,scheme_id,payment_mode,cheque_dd_no,cheque_dd_date,depositing_bank_id,working_limit,scheme_amount,total_amount,status) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$result = $this->db->query($str_query,array($Franchisename,$Address,$Landmark,$Zipcode,$State,$City,$Area,$ResPer,$MobNo,$LandNo,$RetType,$Email,$Other_Area,'Franchise',$date,$ip,$Bank1,$AcNo1,$AcType1,$Org,$PreLang,$Bank2,$AcNo2,$AccType2,$Scheme_id,$PayMode,$ChqDDNo,$ChqDDDate,$DepBank,$WorLimit,$Scheme_amt,$total_amount,'1'));		
		if($result > 0)
		{
