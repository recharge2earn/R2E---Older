...
if($this->Common_methods->CheckAgentBalance($user_id,$Amount) == true)
{
	...
	$orderid_text= $ApiInfo->row(0)->orderid;
	...
	$opid_response_text = $ApiInfo->row(0)->opid_response_text;
		...
		//username=500011&pwd=123&circlecode=2&operatorcode=A&number=980000000&amount=10&orderid=485668&format=json								
...
$buffer = $buffer.$request_url.$parameter;
 ...
 $buffer =  $buffer." METHOD GET ".$buffer = $request_url."?".$parameter;
  
  if ($status == $success_response_text){
    
    $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Success",$buffer,$orderid);
...
elseif($status == $failure_response_text) {
    ...
    
    {
      ...
      $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$mgs","Failure",$buffer,$orderid);
              $this->load->model("Insert_model");
        $this->Insert_model->rechargerefund($recharge_id);
            ...
            {
             return $recharge_id.",Failure,".$mgs.",".$Mobile.",".$Amount.",".$orderid; exit;
            }
     ...
     $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Pending",$buffer,$orderid);
            ...
            {
             return $recharge_id.",Pending,".$operator_trans_id.",".$Mobile.",".$Amount.",".$orderid; exit;
            }
...
$json = json_encode($xml);
 $obj = json_decode($json,TRUE); 
  ...
  $operator_trans_id = $obj[$opid_response_text];
 $mgs = $obj[$message_response_text];
 $buffer = $buffer." METHOD POST ".$request_url."?".$parameter;
