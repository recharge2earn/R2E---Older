<?php
class Do_dmr_model extends CI_Model 
{	
	function _construct()
	{		  
		  parent::_construct();
	...
	public function ProcessRecharge($user_info,$circle_code,$company_id,$Amount,$Mobile,$recharge_type,$service_id,$rechargeBy)
	{
		...
		$this->load->model("Recharge_home_model");
                $this->load->model("dmr_home_model");
...
if($this->Common_methods->CheckAgentBalance($user_id,$Amount) == true)
{
	...
	$url= $ApiInfo->row(0)->url;
			...
			$this->Insert_model->tblewallet_Recharge_DrEntry($user_id,$recharge_id,$transaction_type,$dr_amount,$Description);
					...
					if($ApiInfo->row(0)->api_name == "MYRC")
					{
...
if (preg_match("/Success/i", $status)) {
    ...
    $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Success",$royal_resp,$orderid);
             ...
             return "TXID ".$recharge_id.", Status <b>Success</b>".", Operator ID ".$operator_trans_id.", Number ".$Mobile.", Amount ".$Amount; exit;
...
elseif(preg_match("/Failure/i", $status)) {
      $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Failure",$royal_resp,$orderid);
        ...
        $this->Insert_model->rechargerefund($recharge_id);
               ...
               return "TXID ".$recharge_id.", Status <b>Failure</b>".", Operator ID ".$operator_trans_id.", Number ".$Mobile.", Amount ".$Amount; exit;
...
elseif(preg_match("/ERROR/i", $royal_resp)) {
    $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Failure",$royal_resp,$orderid);
              $this->load->model("Insert_model");
        $this->Insert_model->rechargerefund($recharge_id);
...
elseif(preg_match("/Configuration/i", $royal_resp)) {
    $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Failure",$royal_resp,$orderid);
              $this->load->model("Insert_model");
        $this->Insert_model->rechargerefund($recharge_id);
...
elseif(preg_match("/Authentication/i", $royal_resp)) {
    $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"PLease update API username and password in admin API setting","Failure",$royal_resp,$orderid);
              $this->load->model("Insert_model");
        $this->Insert_model->rechargerefund($recharge_id);
     ...
     $this->Recharge_home_model->updateRechargeStatus($recharge_id,$transaction_id,"$operator_trans_id","Success",$royal_resp,$orderid);
              ...
              return "TXID ".$recharge_id.", Status <b>Success</b>".", Operator ID ".$operator_trans_id.", Number ".$Mobile.", Amount ".$Amount; exit;
					...
					else if($ApiInfo->row(0)->api_name == "Global")
					{
                   	...
                   	{   
					//echo "hiiiiii";exit;
						$req='http://www.jupiterinfosoft.com/jupiterapiportal/api_users/recharge?clientid='.$username.'&password='.$password."&circlecode=".$circle_code."&operatorcode=".$RoyalProvider."&number=".$Mobile.'&amount='.$Amount.'&orderid='.$recharge_id;
