...
{	
	...
	public function getParentCommission($user_id,$from_date,$to_date)
	{
		$str_query = "SELECT IFNULL(SUM(tblparentcommission.amount),0) as comm FROM tblparentcommission
					JOIN tblrecharge ON (tblparentcommission.recharge_id = tblrecharge.recharge_id) 
					where tblrecharge.recharge_status='Success' and tblparentcommission.user_id='$user_id' and  DATE_FORMAT(tblparentcommission.add_date, '%Y-%m-%d') >= '$from_date' and DATE_FORMAT(tblparentcommission.add_date, '%Y-%m-%d') <= '$to_date'";
		//$str_query = "select SUM(amount) as comm from tblparentcommission where user_id = '$user_id' and  and  DATE_FORMAT(add_date, '%Y-%m-%d') >= '$from_date' and DATE_FORMAT(add_date, '%Y-%m-%d') <= '$to_date'";
		$rslt = $this->db->query($str_query);
		return round($rslt->row(0)->comm,4);
	}
	public function getClosing_Balance($user_id,$by_date)
	{				
		$str_query="select IFNULL(sum(amount),0) - (select IFNULL(sum(amount),0) from tblpayment where dr_user_id=? and payment_date<? ) - (select IFNULL(sum(amount),0) as 'RechargeAmount' from tblrecharge where user_id=? and recharge_date<? and (recharge_status='Success' or recharge_status='Pending')) +
		(SELECT IFNULL(SUM(tblparentcommission.amount),0) FROM tblparentcommission
					JOIN tblrecharge ON (tblparentcommission.recharge_id = tblrecharge.recharge_id) 
					where tblrecharge.recharge_status='Success' and tblparentcommission.user_id='$user_id' and DATE_FORMAT(tblparentcommission.add_date, '%Y-%m-%d') < '$by_date' )
		+ (select IFNULL(SUM(commission_amount),0) AS commission_amount from tblrecharge where user_id=? and recharge_date<? and (recharge_status='Success' or recharge_status='Pending')) as CloseBalance from tblpayment where cr_user_id=? and payment_date<?";
