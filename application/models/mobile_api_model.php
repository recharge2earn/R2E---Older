...
{	
	...
	{
		$str_query = "SELECT tblcompany.company_name,tblrecharge.* FROM `tblrecharge`,tblcompany where tblcompany.company_id=tblrecharge.company_id and user_id=(select tu.user_id from tblusers tu where tu.username=? and tu.password = ?) order by recharge_id desc limit 0,50";
	...
	public function GetRechargeStatus($user_id,$gtid)
	{
		...
		if($result->num_rows() == 1 and $format == "json")
		{

                $myObj->txid = $result->row(0)->recharge_id;
                $myObj->status = $result->row(0)->recharge_status;
                $myObj->opid = $result->row(0)->operator_id;
                $myObj->number= $result->row(0)->mobile_no;
                $myObj->amount = $result->row(0)->amount;
                $myObj->orderid = $result->row(0)->orderid;
                return $myJSON = json_encode($myObj);
                }

             elseif($result->num_rows() == 1 and $format == "xml")  //xml
                {
                    header("Content-type: text/xml");
                     $xml_return='<Recharge>';
            $xml_return .='<txid>'.$result->row(0)->recharge_id.'</txid><status>'.$result->row(0)->recharge_status.'</status><opid>'.$result->row(0)->operator_id.'</opid><number>'.$result->row(0)->mobile_no.'</number><amount>'.$result->row(0)->amount.'</amount><orderid>'.$result->row(0)->orderid.'</orderid>';          
    
        $xml_return .= '</Recharge>';
    
        return $xml_return; 
                }
elseif($result->num_rows() == 1 and $format == "csv")
{
return $result->row(0)->recharge_id.",".$result->row(0)->recharge_status.",".$result->row(0)->operator_id.",".$result->row(0)->mobile_no.",".$result->row(0)->amount.",".$result->row(0)->orderid; exit;
}
                else
                {
                     return "Invalid order id/Invalid Response format"; exit;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////		
	...
	public function revertBalance($username,$pwd,$otherUname,$amount,$OPT)
	{
		$this->load->model("common_method_model");	
		...
		if($user_id != false)
		{
			...
			if($otherUserId  != false)
			{
				...
				$cr_user_id = $user_id;
				$resp = $this->Userinfo_methods->validateOTP($dr_user_id,$OPT);
				if($resp == false)
				{
				  echo "Invalid Opt";exit;
				}
	...
	public function AddComplain($username,$pwd,$mobile,$amount,$txid,$message)
	{
		$str_message = $message." Number=".$mobile."Amount=".$amount."Txid".$txid;
		$this->load->model("common_method_model");	
		if($this->common_method_model->getUserId($username,$pwd) != false)
		{
			...
			echo "Your Complain Has been Registered";exit;
		}
		else
		{
