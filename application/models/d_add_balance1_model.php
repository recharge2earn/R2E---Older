<?php
class D_add_balance1_model extends CI_Model 
{	
	function _construct()
	{
	...
	public function add_newbalanceWithLongCode($cr_user_id,$dr_user_id,$amount)
	{
		$this->load->library('common');
		$ip = $this->common->getRealIpAddr();
		$date = $this->common->getDate();
		...
		$str_cr_dr_query = "insert into tblpayment(cr_user_id,dr_user_id,payment_master_id,amount,payment_type,remark,transaction_type,payment_date,payment_time,add_date,ipaddress) values(?,?,?,?,?,?,?,?,?,?,?)";
		$result_cr_dr = $this->db->query($str_cr_dr_query,array($cr_user_id,$dr_user_id,$master_id,$amount,'cash','SMS Direct Payment','Recharge',$payment_date,$payment_time,$date,$ip));
