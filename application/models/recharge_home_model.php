<?php
class Recharge_home_model extends CI_Model 
{	
	function _construct()
	{
	...
	public	function add($company_id,$amount,$mobile_no,$user_id,$service_id,$description,$recharge_type,$recharge_status,$ApiInfo,$scheme_id,$rechargeBy)
	{
		...
		$user_type = $this->Userinfo_methods->getUserType($user_id);
			...
			$commission_per = 0+$this->get_schem_commission($scheme_id,$company_id);
		 
			$commission_amount = $amount * $commission_per/ 100;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(trim($user_type) == 'Agent')
			{
				...
				$scheme_id = $this->schem_id_parent($parent);
				...
				if($this->getUserType($parent) == "Distributor")
				{
					...
					$commission_d_per = 0+$this->get_schem_commission($scheme_id,$company_id);
						$commission_d_amount =$amount * $commission_d_per/100;
				...
				$parentOf_d = $this->GetDistributerID($parent); 
					$scheme_id = $this->schem_id_parent($parentOf_d);
				...
				if($this->getUserType($parentOf_d) == "MasterDealer")
				{
					...
					{
					   $mdComm = 0+$this->get_schem_commission($scheme_id,$company_id);
						
					
					
						$commission_MDealer_amount = $amount * $mdComm/ 100;
						$this->addParentCommission($parentOf_d,$user_id,$commission_MDealer_amount,$mdComm,$recharge_id,$company_id);
					}
			...
			else if(trim($user_type) == 'Distributor')
			{
				...
				if($this->getUserType($parent) == "MasterDealer")
				{
					...
					{
						$mdealercomm = 0+$this->get_schem_commission($scheme_id,$company_id);
					
					
						$commission_m_amount = $amount * $mdealercomm/ 100;
						$this->addParentCommission($parent,$user_id,$commission_m_amount,$mdealercomm,$recharge_id,$company_id);
					}
				}
...
return round($result->row(0)->Balance,4);	
	...
	public function updateRechargeStatus($recharge_id,$trns_id,$operator_trans_id,$status,$orderid,$buffer)
	{
		$str_query = "update tblrecharge set recharge_status=?,transaction_id=?,operator_id=?,response=?,orderid=? where recharge_id = ?";
		$rslt = $this->db->query($str_query,array($status,$trns_id,$operator_trans_id,$orderid,$buffer,$recharge_id));
	...
	public function get_schem_commission($scheme_id,$company_id){
	    
	 $rslt = $this->db->query("select * from tblcommission where scheme_id='$scheme_id' and company_id='$company_id'");
		return $rslt->row(0)->royalComm;   
	}
	
		public function schem_id_parent($parent)
	{		
	$str_query = "select * from tblusers where user_id = ?";
	$result = $this->db->query($str_query,array($parent));		
	return $result->row(0)->scheme_id;	
	}
