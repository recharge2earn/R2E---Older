<?php
class D_list_retailer_model extends CI_Model 
{	
	function _construct()
	{
	...
	{
		$str_query = "select tblusers.*,(select city_name from tblcity where tblcity.city_id=tblusers.city_id) as city_name,(select state_name from tblstate where tblstate.state_id 	=tblusers.state_id) as state_name  from tblusers where parent_id=? and usertype_name='Retailer' order by business_name";
		$result = $this->db->query($str_query,array($user_id));
		return $result;
	}
	public function get_retailer_limited($start_row,$per_page,$user_id)
	{
		$str_query = "select tblusers.*,(select city_name from tblcity where tblcity.city_id=tblusers.city_id) as city_name,(select state_name from tblstate where tblstate.state_id 	=tblusers.state_id) as state_name  from tblusers where parent_id=? and usertype_name='Retailer' order by business_name limit $start_row,$per_page";
		$result = $this->db->query($str_query, array($user_id));
		return $result;
	}	
