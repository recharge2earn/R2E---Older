  ...
  <div class="container">
    <div class="panel panel-primary">
      ...
      <div class="panel-body">     
       ...
       <form action="<?php echo base_url()."dth"; ?>" method="post" name="frmRecharge" id="frmRecharge" autocomplete="on" onsubmit="return validation();"> 
    ...
    <input type="hidden" name="hidCircle" id="hidCircle" />      
     ...
     <input class="form-control" required="" id="txtMobileNo" name="txtMobileNo" value="txtMobileNo" data-toggle="tooltip" title="Please Enter DTH Number" placeholder="Please Enter DTH Number*" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" type = "number" maxlength = "12"/>
     </div>
     
     
      <div class="form-group">
       <select class="form-control" required="" id="ddlOperator" name="ddlOperator" onChange="SetParam()"  title="Select Operator.<br />Click on drop down"><option value="0">Select Operator*</option>
 <?php
    $str_query = "select * from tblcompany where service_id='2' and company_id != 34 and company_id != 39  order by company_name";
...
<input type="submit" id="btnRecharge" name="btnRecharge" class="btn btn-primary" value="Recharge"><span id="wait_tip" style="display:none;"> Please wait...<img src="images/ajax-loader2.gif"
           id="loading_img"/></span> <!--| <button type="button" name="search" id="search" class="btn btn-danger">DTH Info</button>-->
