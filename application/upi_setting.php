...
class Upi_Setting extends CI_Controller {
	...
	public function pageview()
	{
		...
		$config['base_url'] = base_url()."upi_setting/pageview";
		...
		$this->load->view('upi_setting_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 
			...
			{				
				...
				if($this->update($upi_id) == true)
				{
					$this->msg ="UPI Updated Successfully.";
					$this->pageview();
				}
				else
				{
	...
	public	function update($upi_id)
	{	
		$str_query = "update tblusers set upi_id='$upi_id' where user_id='100'";
	...
	{
		$str_query = "select upi_id from  tblusers where user_id='100'";
		$result = $this->db->query($str_query);
		return $result;
	}
	public function get_api_limited($start_row,$per_page)
	{
		$str_query = "select upi_id from  tblusers where user_id='100'  limit $start_row,$per_page";
		$result = $this->db->query($str_query);
		return $result;
	}	
